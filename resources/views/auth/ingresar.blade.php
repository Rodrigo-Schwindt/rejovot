    ...
    <div class="w-full max-w-[440px]">
        ...
        <div class="rounded-[4px] bg-white p-7 shadow-[0_20px_50px_rgba(7,27,61,.35)]">
            ...
            <form method="POST" action="{{ route('ingresar.post') }}" class="mt-6 space-y-4">
                ...
                <div>
                    ...
                    <input id="login" type="text" name="login" value="{{ old('login') }}" required autofocus
                           ...
                           class="h-[46px] w-full rounded-[4px] border border-slate-300 px-3 text-[15px] text-slate-800 outline-none transition focus:border-[#002B56]">
                </div>
                ...
                <label class="flex items-center gap-2 text-[14px] text-slate-600">
                    <input type="checkbox" name="remember" value="1" class="h-4 w-4 rounded border-slate-300 accent-[#002B56]">
                    Mantener la sesión iniciada
                </label>

                <button type="submit"
                        class="h-[48px] w-full cursor-pointer rounded-[4px] bg-[#002B56] text-[15px] font-bold uppercase tracking-wide text-white transition hover:bg-[#0A2249]">
                    Ingresar
                </button>
            </form>
            ...
            <p class="mt-5 border-t border-slate-100 pt-4 text-center text-[13px] text-slate-500">
                ...
                <a href="mailto:{{ $contact?->mail_adm ?: 'ventas@rejovot.com.ar' }}" class="font-semibold text-[#002B56]">{{ $contact?->mail_adm ?: 'ventas@rejovot.com.ar' }}</a>
            </p>
        </div>
